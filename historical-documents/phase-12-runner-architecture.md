# Phase 12: Runner Architecture Refactor (12.0 - 12.7)

> **Status**: COMPLETE — every phase from 12.0 through 12.7 landed and was
> signed off by LazyAF's own dogfood pipeline. Retired from PLAN.md on
> 2026-08-30 (itself a Phase 12.8 line item) so that the live plan is about what
> is next. **Phase 12.8 is still open and stays in [PLAN.md](../PLAN.md).**
>
> **Goal**: replace the polling runner pool with ephemeral, per-step containers
> on persistent workspaces — local or remote — and build the spec/eval layer
> alongside it.

## What this document is

Three layers of record, in the order they were written:

1. **The attempt-#3 phase plans** — the deliverables and EXIT GATE for each
   phase, as agreed 2026-08-29. This is the contract each phase was held to.
2. **The architecture reference** — target architecture, workspace transfer
   protocol, `continue_in_context` semantics, lifecycle state machines, and the
   test-infrastructure prerequisites. Written January 2026; the code is the
   authority now, but this explains *why* the code is shaped as it is.
3. **The January-era per-phase specs** for 12.0-12.7, including the original
   test-first tables. Kept because the test names in them are largely the names
   that exist in `tdd/` today.

A superseded fourth section closes the file: the January plan for 12.8, which
`upcoming/wave10-v1-retirement.md` replaced.

## Outcome summary

| Phase | Landed | Evidence |
|-------|--------|----------|
| 0 — Self-hosting bootstrap | 2026-08-29 | A push to the internal remote runs the tiered dogfood pipeline; `scripts/ci_gate.py` enforces `tdd/skip_baseline.json` plus the per-tier floors in `tdd/tier_floors.json` |
| 12.0 — Unify runner entrypoints | 2026-08-30 | `images/agent-base/Dockerfile` installs `runner-common` system-wide and **asserts the import at build time** (`RUN ... python3 -c "import runner_common.agent_wrapper as w; assert callable(w.main)"`); `images/claude` and `images/gemini` inherit from it; the three monolithic entrypoints are deleted (commit `67a4e1c`) |
| 12.1 — LocalExecutor + step state machine | — | `StepExecution` with a unique `execution_key` for idempotency, `StepStateMachine`, `recover_orphaned_executions()` at startup |
| 12.2-INT — Workspace persistence + executor wiring | 2026-08-29 | Run `71d56980` (push-triggered) executed all 6 steps in ephemeral containers on a persistent workspace volume; `verify_executor` confirmed `executor='local'` for every one |
| 12.2.5 — Specification data model | 2026-08-29 | `backend/app/models/spec.py`, migration `0003_spec_layer.py`, `/api/features`, `/api/user-stories`, `/api/criteria`, `/api/prompt-templates`; the three north-star stories seeded |
| 12.3 — Control layer & step images | 2026-08-29 | `lazyaf-{base,claude,test-runner}:dev` built by `scripts/build_images.py` with content-hash labels; the in-container runtime reports to `/api/steps/*`; run #11 passed the gate |
| 12.2.6 — Test result tie-back | 2026-08-30 | A push-triggered run wrote a TestRun joined to criterion `fb95f11d` (`passed` / commit `2a513dd4` / branch `main` / `us1.pipeline-outcome-gates-branch`); 9 TestRefs linked, 0 orphans. Migration `0004_test_tieback.py` |
| 12.4 — Script/docker steps fully ephemeral | 2026-08-30 | Script/docker execution deleted from the three images **and** from `runner-common`; the interim DooD anchor retired |
| 12.5 — Agent steps via the control layer | 2026-08-30 | `StepUsage` (migration `0005_step_usage.py`) live on the gate at 1163/7 tokens; runners idle on every default path (asserted) |
| 12.6 — RemoteExecutor + runner agents | 2026-08-30 | The ported contract suite executes at zero skips; the polling stack is deleted and policed by `tdd/unit/services/test_no_legacy_code.py`; all seven polling endpoints 404 on a live probe. Migrations `0006_runner_registry.py`, `0007_drop_polling_runner_columns.py` |
| 12.6.5 — Experiments & leaderboards | 2026-08-30 | `backend/app/models/experiment.py`, migration `0010_experiments.py`, `GET /api/experiments/{id}/leaderboard`, `GET /api/leaderboards/feature/{id}`; experiment finalize is one atomic CAS |
| 12.6.6 — Spec-curated agent context | 2026-08-30 | `backend/app/routers/spec_context.py`, `GET /api/cards/{id}/spec-context`; `card_id` now resolves from step config as well as from run context |
| 12.7 — Debug re-run mode | 2026-08-30 | `backend/app/routers/debug.py`, migration `0009_debug_sessions.py`, `images/debug-sidecar`, `WS /api/debug/{id}/terminal`, `lazyaf debug` shipped in the wheel |

Landing commits: `ecdeb0c`, `a55c49e`, `67a4e1c`, `4b429c6`, `a39cb24`,
`db5f9f5`, `5334b09`, `7d611fd`.

The abandoned first attempt lives on branch `failure_01` (reference only —
never merge it); its post-mortem and salvage map are in
`upcoming/failure_01-salvage-audit.md`.

Per-phase wiring documents, which carry the implementation-level detail and the
review findings for each wave, are in `upcoming/`: `wave2-123-wiring.md`,
`wave4-125-wiring.md`, `wave5-126-wiring.md`, `wave6-1265-wiring.md`,
`wave6-1266-wiring.md`, `wave6-127-wiring.md`.

---

# Part 1 — Attempt-#3 phase plans (deliverables and exit gates)

> Verbatim from PLAN.md as agreed 2026-08-29. Every gate below was met; see the
> outcome summary above for the evidence.
...
# Parts 2 and 3 — Architecture reference, and the January-era per-phase specs

> The as-built system is the authority now; what follows is why it is shaped the
> way it is, and the test-first tables the phases were written against. Section
> headings are as they stood in PLAN.md.
...
# Part 4 — SUPERSEDED: the January plan for Phase 12.8

> Kept for the record only. **Do not work from the checklist below.** Most of it
> landed under other phases — the legacy-removal tests are
> `tdd/unit/services/test_no_legacy_code.py`, and the old entrypoints and
> `runner_pool.py` were removed in 12.4 and 12.6 — and the remainder was
> replaced by `upcoming/wave10-v1-retirement.md`, which is the plan of record
> for 12.8.
