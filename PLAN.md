...
| Thing | Value | Source |
|---|---|---|
| T1 (unit + non-Docker integration) | **5710 passed / 0 failed / 0 errors, 1 baselined skip** (executed = 5710) | Measured 2026-09-11. **Floor RAISED to 5595** (`tdd/tier_floors.json`), closing a 22% margin that had been open for three waves — at 4432 against 5710, a 1,278-test subtree could have stopped collecting with the gate still printing OK. +189 of the growth is `runner-agent/tests`, folded into T1's selection by the same change after running in **no tier at all** (PLAN.md's own L3-3). Stamped from a GREEN run (`CI GATE [T1]: OK`), but it took four attempts: **the tier is not reliably green.** Five timing-sensitive tests flaked across earlier runs, never the same one twice, each passing in isolation; the floors file names all five and the two root causes. `executed` was 5710 on every run regardless, which is the quantity the floor gates. The flakiness is tracked as its own defect rather than absorbed. |
| T2 (Docker integration) | **83 passed / 0 failed / 0 errors, 1 baselined skip** (executed = 83) | Measured 2026-09-11 on a fully GREEN run in 7m01s; `CI GATE [T2]: OK`. **Floor RAISED 75 -> 81**, exactly the raise the previous note pre-computed. No new T2 coverage landed — this is the overdue ratchet on coverage that already existed. |
| T3 (e2e quick) | **22 passed / 0 failed / 0 skipped** (executed = 22), measured three times | Measured 2026-09-11, green on all three runs. Floor stays **21** — 22 minus ~2% is 21.56, so 21 is already correct and raising would be ratcheting on noise; only `measured` moved. **Skips went 3 -> 0**: the three baselined `test_graph_pipeline.py` legacy-format tombstones were deleted at 12.8 P6 along with their `skip_baseline.json` entry, because their stated reason finally became true. A 2026-09-10 survey called T3 a ~50% flake; it did not reproduce in three runs, which does not clear the shared-session defect (that one bites T1) but does mean T3 is not where it shows. |
