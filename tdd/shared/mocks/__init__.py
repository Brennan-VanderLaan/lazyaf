...
class MockContainer:
    ...
    # Simulation options
    run_duration: float = 0.0  # How long the container "runs" before exiting
    simulated_exit_code: int = 0  # Exit code when container completes normally
    ...
    def __post_init__(self):
        ...
        if isinstance(self.command, list):
            # Direct sleep command: ["sleep", "N"]
            if len(self.command) >= 1 and self.command[0] == "sleep":
                # Long running command - simulate long duration for timeout testing
                self.run_duration = 3600.0

            # Shell commands: ["sh", "-c", "..."]
            if len(self.command) >= 3 and self.command[0] == "sh" and self.command[1] == "-c":
                cmd_str = self.command[2]
                if cmd_str.startswith("exit "):
                    try:
                        self.simulated_exit_code = int(cmd_str.split()[1])
                    except (ValueError, IndexError):
                        pass
                elif "sleep" in cmd_str:
                    # Long running command - simulate long duration
                    self.run_duration = 3600.0
                elif "kill" in cmd_str:
                    # Self-kill command - simulate signal termination
                    # Exit code 137 = 128 + 9 (SIGKILL)
                    self.simulated_exit_code = 137
    ...
    def start(self):
        """Start the container."""
        if self.status not in (MockContainerState.CREATED, MockContainerState.EXITED):
            raise RuntimeError(f"Cannot start container in {self.status} state")
    ...
    def logs(
        ...
        When stream=True and follow=True, returns an iterator that yields log lines.
        The iterator blocks until the container exits (simulated).
        """
        if stream and follow:
            # Return a streaming iterator that simulates following logs
            return MockLogStream(self)
    ...
    def _complete_execution(self):
        """Mark container as completed with simulated exit code."""
        if self.status == MockContainerState.RUNNING:
            self.status = MockContainerState.EXITED
            self.exit_code = self.simulated_exit_code
            self._update_attrs()


class MockLogStream:
    """
    Mock log stream iterator that simulates following container logs.

    Yields any initial log content, then waits for the container to exit.
    Used when logs(stream=True, follow=True) is called.
    """

    def __init__(self, container: MockContainer):
    ...
    def __next__(self):
        ...
        # First, yield any existing log content
        if not self._yielded_content and self.container.logs_content:
            self._yielded_content = True
            return self.container.logs_content.encode()

        # Check if container has exited (killed, stopped, etc.)
        if self.container.status in (MockContainerState.EXITED, MockContainerState.DEAD):
            raise StopIteration

        # Check if simulated run duration has elapsed
        elapsed = time.time() - self._start_time
        if self.container.run_duration > 0 and elapsed >= self.container.run_duration:
            # Container completed normally
            self.container._complete_execution()
            raise StopIteration

        # For short-running containers (run_duration == 0), complete immediately
        if self.container.run_duration == 0:
            self.container._complete_execution()
            raise StopIteration

        # Container still running - sleep briefly then check again
        # This simulates blocking on log output
        time.sleep(0.1)
...
class MockContainersAPI:
    ...
    def create(
        ...
        **kwargs,
        ...
        if self._client.fail_next_run:
            self._client.fail_next_run = False
            raise DockerException("Failed to create container")
        ...
        container = MockContainer(
            ...
            name=container_name,
            image=image,
            status=MockContainerState.CREATED,
            command=command,
            environment=environment or {},
            volumes=volumes or {},
        )
